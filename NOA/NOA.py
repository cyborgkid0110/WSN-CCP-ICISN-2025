            ...
            for i in range(nPop):
                ...
                if random.random() < Pa1:
                    for j in range(dim):
                        ...
                        if random.random() > random.random():
                            ...
                            # if random.random() > random.random():
                            temporary_pos = pop[i, j]
                            pop[i, j] = pop[cv2, j] + mu * (pop[cv, j] - pop[cv1, j]) + mu * (random.random() < Alpha) * (r2 * r2 * ub[j] - lb[j])
                            pop[i, j] = max(lb[j], min(pop[i, j], ub[j]))
                            newFit = fitness_function(pop[i, :], nNode, Rs, VarMax + 1, VarMax + 1, ban_position)
                            if (check_connectivity(pop[i, :], nNode, Rc) == False or newFit > currentFit):
                                pop[i, j] = temporary_pos
                ...
                # Exploitation phase 1
                    ...
                    if random.random() < random.random():
                        ...
                        for j in range(dim):
                            temporary_pos = pop[i, j]
                            pop[i, j] = pop[i, j] + mu * abs(RL[i, j]) * (bestSol[j] - pop[i, j]) + r1 * (pop[cv, j] - pop[cv1, j])
                            pop[i, j] = max(lb[j], min(pop[i, j], ub[j]))
                            newFit = fitness_function(pop[i, :], nNode, Rs, VarMax + 1, VarMax + 1, ban_position)
                            if (check_connectivity(pop[i, :], nNode, Rc) == False or newFit > currentFit):
                                pop[i, j] = temporary_pos
                        ...
                        for j in range(dim):
                            # if random.random() > random.random():
                            temporary_pos = pop[i, j]
                            pop[i, j] = bestSol[j] + mu * (pop[cv, j] - pop[cv1, j])
                            pop[i, j] = max(lb[j], min(pop[i, j], ub[j]))
                            newFit = fitness_function(pop[i, :], nNode, Rs, VarMax + 1, VarMax + 1, ban_position)
                            if (check_connectivity(pop[i, :], nNode, Rc) == False or newFit > currentFit):
                                pop[i, j] = temporary_pos
